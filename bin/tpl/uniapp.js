...
module.exports = async function initYipackTemplateUniapp() {
    let spinner = ora();
    spinner.start(chalk.green('yipack-template-uniapp模板下载中...'));
    try {
        ...
        fs.removeSync(myConfig.tempDir);
        spinner.succeed(chalk.green('yipack-template-uniapp模板下载成功'));
    } catch (err) {
        spinner.fail(chalk.red('yipack-template-uniapp模板下载失败'));
        spinner.stop();
        console.log(err);
    }
};
